...
class Deux_Kirki {
	...
	public function enqueue_styles() {
		...
		if ( ! empty( $styles ) ) {
			...
			wp_add_inline_style( $current_theme->stylesheet, $styles );
		}
	}
	...
	public function enqueue_fonts() {
		...
		foreach ( self::$fields as $field ) {
			...
			if ( isset( $value['font-family'] ) && ( 'ArcaMajora' !== $value['font-family'] ) ) {
				...
				if ( $url_is_valid ) {
					wp_enqueue_style( $key, esc_url_raw( $url ), null, null );
				}
			}
